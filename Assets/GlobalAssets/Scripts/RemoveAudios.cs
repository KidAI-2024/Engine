...
public class RemoveAudios :  MonoBehaviour
{
public List<AudioClip> capturedAudios;
...
void Start()
{
    ...
    capturedAudios = GrandParentObject.GetComponent<MicController>().capturedAudios;
}
...
public void RemoveThisImage()
{
    ...
    for (int i = parent.childCount - 1; i > ImageIndex; i--)
    {
        ...
        parent.GetChild(i).GetComponent<RemoveAudios>().ImageIndex = i - 1;
    }

    // Remove the last child (empty space)
    Destroy(gameObject);
    // Update the capturedImages list
    Debug.Log("RecordIndex: " + ImageIndex + " capturedRecords.Count: " + capturedAudios.Count);

    capturedAudios.RemoveAt(ImageIndex);
}
}
